...
def dashboard():
    ...
    if not token:
        ...
        return redirect(f'{AUTH_SERVER}/login?service=https://noctiservice1.vercel.app/dashboard')
    ...
    if response.json().get('valid'):
        return jsonify({"message": f"Welcome to NoctiService 1, {response.json()['username']}!"})
    else:
